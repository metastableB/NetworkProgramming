...
void q_delay_src_vs_burst_time_delta();
void out_queue_vs_time();
void switch_queue_vs_time();
...
int main(){
	...
	#elif defined(Q_DELAY_SRC_VS_BURST_TIME_DELTA)
		q_delay_src_vs_burst_time_delta();
	#elif defined(P_LOST_SW_VS_Q_SIZE)
		packet_lost_sw_vs_q_size();
	#elif defined(OUT_QUEUE_VS_TIME)
		out_queue_vs_time();
	#elif defined(SWITCH_QUEUE_VS_TIME)
		switch_queue_vs_time();
	#else
		dummy_test();
	#endif
	//delete sw;
...
void packet_lost_src_vs_q_size(){
	...
	for(int i = 0; i < DATA_POINTS; i++) {
		...
		/* initializing source */
		max_q = max_q + INCREMENTS*MULTIPLIER;
...
void q_delay_src_vs_p_sending_rate(){
	...
	for(int i = 0; i < DATA_POINTS; i++) {
		...
		std::vector<Source> v_src;
		/* initializing source */
		max_p_s_r = max_p_s_r + INCREMENTS*MULTIPLIER;
		for(int i = 0; i < NUM_SOURCES; i++){
			v_src.push_back(
				Source(max_p_s_r,
				SRC_LINK_BW,
...
void q_delay_src_vs_burst_size() {
	...
	for(int i = 0; i < DATA_POINTS; i++) {
		...
		std::vector<Source> v_src;
		/* initializing source */
		max_bs = max_bs + INCREMENTS*MULTIPLIER;
		for(int i = 0; i < NUM_SOURCES; i++){
			v_src.push_back(
				Source(SRC_SENDING_RATE,
				SRC_LINK_BW,
...
void q_delay_src_vs_burst_time_delta() {
	std::cout << "Logging average queuing delay at src vs burst time delta\n";
	FILE* fp;
	...
	fp = fopen(OUTPUT_FILE,"w");
	auto max_btd = SRC_BURST_TIME_DELTA;
	...
	for(int i = 0; i < DATA_POINTS; i++) {
		...
		std::vector<Source> v_src;
		/* initializing source */
		max_btd = max_btd + std::chrono::microseconds(INCREMENTS*MULTIPLIER);
		for(int i = 0; i < NUM_SOURCES; i++){
			v_src.push_back(
				Source(SRC_SENDING_RATE,
				SRC_LINK_BW,
		...
		Handler handler(SIM_DURATION,
			...
			sw);
		handler.simulate();
		fprintf(fp,"%ld\t%lld\n",max_btd.count(),handler.average_queuing_delay.count()/1000000);
	}
	fclose(fp);
	std::string args = std::string(OUTPUT_FILE) + " Burst_Time_Delta " +"Average_queuing_delay[ms]";
	std::string title = " Link_bw:_"+std::to_string(SRC_LINK_BW)+"bps,_Queue_Size_" + std::to_string(SRC_MAX_Q_SIZE);
	title += ",_Sending_rate_"+std::to_string(SRC_SENDING_RATE);
	std::string img_name = " q_delay_src_vs_burst_time_delta";
	std::string command = "python ./plotter.py " + args + title + img_name;
	std::system(command.c_str());
}
void packet_lost_sw_vs_q_size(){
	...
	for(int i = 0; i < DATA_POINTS; i++) {
		...
		max_sw_q = max_sw_q + INCREMENTS*MULTIPLIER;
...
void out_queue_vs_time(){
	std::cout << "Logging Out Queue size vs time\n";
	FILE* fp;
	fp = fopen(OUTPUT_FILE,"w");
	if(SW_OPERATING_MODE == Switch::switch_operating_mode::TDM)
		throw std::runtime_error("Cannot simulate in BURSTY/TDM\n");
	...
	fclose(fp);
	std::string args = std::string(OUTPUT_FILE) + " Time[ms] " +"Out_Queue_Size";
	std::string title = " Link_bw:_"+std::to_string(SRC_LINK_BW)+"bps,_Out_Queue_Mac_" + std::to_string(SRC_MAX_Q_SIZE);
	std::string img_name = " Out_Queue_Size_vs_Time";
	std::string command = "python ./plotter.py " + args + title + img_name;
	std::system(command.c_str());
}

void switch_queue_vs_time(){
	std::cout << "Logging Switch Queue size vs time\n";
	FILE* fp;
	fp = fopen(OUTPUT_FILE,"w");
	if(SRC_CONNECTION_TYPE == Source::connection_type::BURSTY ||
		SW_OPERATING_MODE == Switch::switch_operating_mode::TDM)
	...
	fclose(fp);
	std::string args = std::string(OUTPUT_FILE) + " Time[ms] " +"Switch_Queue_Size";
	std::string title = " Link_bw:_"+std::to_string(SRC_LINK_BW)+"bps,_Switch_Queue_Max_" + std::to_string(SW_Q_SIZE_MAX);
	std::string img_name = " Switch_Queue_Size_vs_Time";
	std::string command = "python ./plotter.py " + args + title + img_name;
	std::system(command.c_str());
}
